...
using NaveStarWars.ViewModels;
using NaveStarWars.Entidades;
using NaveStarWars.Dao;

namespace NaveStarWars.Servicos
{
    public class SincronizadorService
    {
        ...
        private async Task SincronizarNaves()
        {
            ...
            do
            {
                resultadoApi = await httpClient.GetFromJsonAsync<ResultadoApi<NaveViewModel>>(resultadoApi?.Next ?? URL_PLANETAS);
                lista.AddRange(resultadoApi.Results);
            } while (resultadoApi.Next != null);
